...
echo "$($PSQL "SELECT AVG (winner_goals):: NUMERIC(3,2) FROM games")"

echo -e "\nAverage number of goals in all games from both teams:"
echo "$($PSQL "SELECT AVG(winner_goals + opponent_goals) AS avg FROM games")"

echo -e "\nMost goals scored in a single game by one team:"
echo "$($PSQL "SELECT MAX(GREATEST(winner_goals, opponent_goals)) AS max FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(*) FROM games WHERE winner_goals > 2")"

echo -e "\nWinner of the 2018 tournament team name:"
echo "$($PSQL "SELECT teams.name FROM teams JOIN games ON teams.team_id = games.winner_id WHERE games.round='Final' AND games.year=2018")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
query=$(cat << END
SELECT teams.name FROM teams
JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id 
WHERE games.round='Eighth-Final' AND games.year=2014 ORDER BY teams.name
END
)
echo "$($PSQL "$query")"
...
query=$(cat << END
SELECT games.year, teams.name FROM games 
